  ...
  actions: {
    ...
    async loadMessages(conversationId) {
      ...
      try {
        ...
        this.errors.messages[conversationId] = error.response?.data?.message || 'Không thể tải tin nhắn'
    ...
    setActiveConversation(conversationId) {
      ...
      if (conversationId && !this.messagesByConversation[conversationId]) {
    ...
    handleIncomingMessage(messageData) {
      ...
      const messages = this.messagesByConversation[conversationId]
      const tempIndex = messages.findIndex(m => m.isTemporary && m.content === messageData.content)
      if (tempIndex !== -1) {
      ...
      if (conversationId !== this.activeConversationId) {
        this.unreadByConversation[conversationId] = (this.unreadByConversation[conversationId] || 0) + 1
        this.calculateTotalUnreadCount()
      }
    ...
    calculateTotalUnreadCount() {
      this.totalUnreadCount = Object.values(this.unreadByConversation).reduce((sum, count) => sum + count, 0)
    },
