  ...
  actions: {
    async fetchUserPoints() {
      ...
      try {
        ...
        notification.showError('Không thể tải thông tin điểm thưởng');
      } finally {
    ...
    async redeemReward(rewardId, pointsToSpend) {
      ...
      try {
        ...
        notification.showSuccess(`Đổi thưởng thành công! Voucher: ${result.voucherCode}`);
        return result;
        
      } catch (error) {
        console.error('Failed to redeem reward:', error);
        notification.showError('Đổi thưởng thất bại. Vui lòng thử lại!');
        throw error;
      } finally {
    ...
    async awardPoints(action, metadata = {}) {
      try {
        ...
        if (result.pointsAwarded > 0) {
          ...
          const notification = useNotificationStore();
          notification.showSuccess(`+${result.pointsAwarded} điểm thưởng!`);
        }
